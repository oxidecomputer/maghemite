    ...
    (
        ...
        Sample::new(
            &DdmSession {
                ...
                switch_fab: $switch_idents
                    .fab
                    .clone()
                    .map(|c| c.to_string())
                    .unwrap_or($switch_idents.target.to_string())
                    .into(),
                switch_lot: $switch_idents
                    .lot
                    .clone()
                    .map(|c| c.to_string())
                    .unwrap_or($switch_idents.target.to_string())
                    .into(),
                switch_wafer: $switch_idents.wafer.unwrap_or(0),
                switch_wafer_loc_x: $switch_idents
                    .wafer_loc
                    .map(|[x, _]| x)
                    .unwrap_or(0),
                switch_wafer_loc_y: $switch_idents
                    .wafer_loc
                    .map(|[_, y]| y)
                    .unwrap_or(0),
