...
pub struct BgpSpec {
    ...
    /// (bestpath fanout). Absent means 1: a single best path.
    #[serde(default)]
    pub max_paths: Option<std::num::NonZeroU8>,
}
...
/// updated in place. The daemon-owned "default" router is never torn down:
/// when absent its configuration is emptied instead.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
