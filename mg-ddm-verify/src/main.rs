...
async fn main() -> Result<()> {
    let sleds: Vec<Sled> = serde_json::from_str(&std::fs::read_to_string("sleds.json")?)?;
    ...
    for sled in &all_sleds {
        ...
        let originated: Vec<Destination> = serde_json::from_str(
            &reqwest::get(format!("http://{}:8000/originated", &sled_to_ip[sled]))
                .await?
                .text()
                .await?,
        )?;
    ...
    let mut missed_direction: HashMap<String, HashSet<String>> = HashMap::default();
