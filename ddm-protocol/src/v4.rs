...
pub struct TunnelOrigin {
    ...
    #[serde(default)]
    pub metric: u64,
    /// Identity of the router instance that originated this tunnel endpoint.
    /// `None` when the origin came from a peer or client that predates
    /// multi-router support.
    ///
    /// Skipped when absent so that the serialized form of an id-less origin
    /// is byte-identical to the v3 form (ddmd persists origins keyed by
