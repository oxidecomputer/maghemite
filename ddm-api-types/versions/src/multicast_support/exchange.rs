...
//! Exchange (wire) types added in version 2 (MULTICAST_SUPPORT).
...
// Currently used for loop detection: if our router_id appears in the
// path, the announcement has already traversed us and is dropped. The
// path structure also carries topology information for future
// replication optimizations (RFD 488).
#[derive(
    Debug, Clone, PartialEq, Eq, Hash, Deserialize, Serialize, JsonSchema,
)]
